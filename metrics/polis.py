...
class PolisEval():
    ...
    def evaluateImg(self, imgId):
        ...
        gt_bboxs = [bounding_box(np.array(gt['segmentation'][0]
                                          ).reshape(-1, 2)) for gt in gts]
        dt_bboxs = [bounding_box(np.array(dt['segmentation'][0]
                                          ).reshape(-1, 2)) for dt in dts]
        gt_polygons = [np.array(gt['segmentation'][0]).reshape(-1, 2)
                       for gt in gts]
        dt_polygons = [np.array(dt['segmentation'][0]).reshape(-1, 2)
                       for dt in dts]
        gt_bboxs = [bounding_box(np.array(gt['segmentation'][0]
                                          ).reshape(-1, 2)) for gt in gts]
        # IoU match
        ...
        for i, gt_poly in enumerate(gt_polygons):
            ...
            if iou > 0.5:  # iouThres:
                polis = compare_polys(Polygon(gt_poly),
                                      Polygon(dt_polygons[matched_idx]))
                img_polis_avg += polis
                num_sample += 1
    ...
    def evaluate(self):
        ...
        print(f'average polis: {polis_avg}\n')

        return polis_avg

    def evaluateIns(self, imgId, insId):
        gts = self._gts[imgId]
        dts = self._dts[imgId]
        ...
        iou = ious[matched_idx, 0]
        if iou > 0.5:  # iouThres:
            polis = compare_polys(Polygon(gt_polygons[0]),
                                  Polygon(dt_polygons[matched_idx]))
            return polis, iou
        else:
            return -1, iou
